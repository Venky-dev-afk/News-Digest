...
// Initialize Nhost Client correctly
const nhost = new NhostClient({
  subdomain: "ldexggoppwitfgltnaoo",
  region: "ap-south-1",
});

// If authentication is required, get the token
const authToken = await nhost.auth.getSession()?.accessToken; // Ensure user is logged in
...
async function fetchNews() {
    const authToken = await nhost.auth.getSession()?.accessToken; // Fetch dynamically
    ...
    try {
      const response = await fetch("https://ldexggoppwitfgltnaoo.hasura.ap-south-1.nhost.run/v1/graphql", {
        ...
        headers: {
          ...
          ...(authToken ? { "Authorization": `Bearer ${authToken}` } : {}) // Include token only if available
        },
